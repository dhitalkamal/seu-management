"""Unit tests for SoftDeleteOrganizationUseCase."""
...
def test_soft_delete_sets_deleted_at():
    """SoftDeleteOrganizationUseCase sets deleted_at to a non-None timestamp."""
    org = make_org(status="active")
    repo = FakeOrgRepository([org])
    SoftDeleteOrganizationUseCase(repo).execute(org_id=org.id)
...
def test_soft_delete_missing_org_raises():
    ...
    with pytest.raises(OrgNotFoundError):
        SoftDeleteOrganizationUseCase(repo).execute(org_id=uuid.uuid4())
